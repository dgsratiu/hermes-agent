...
class HermesCLI:
    ...
    def _handle_omega_goal_command(self, cmd: str):
        """Handle /omega-goal and /omega via the shared omega module."""
        try:
            from hermes_cli.omega_goal import run_slash

            output = run_slash(cmd, session_id=self.session_id)
        except Exception as exc:  # pragma: no cover - defensive UI boundary
            output = f"(._.) omega error: {exc}"
        if output:
            print(output)
    ...
    def process_command(self, command: str) -> bool:
        ...
        elif canonical in {"omega-goal", "omega"}:
            self._handle_omega_goal_command(cmd_original)
