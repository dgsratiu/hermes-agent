...
only renders as a voice bubble when explicitly flagged) and the Telegram
artifact rule that explicit final-response ``MEDIA:/absolute/path``
attachments go through sendDocument instead of photo/audio/video methods.
"""
...
@pytest.mark.parametrize(
    "filename",
    ["artifact.png", "speech.mp3", "clip.mp4", "notes.txt"],
)
async def test_base_adapter_routes_telegram_media_tags_to_document_sender(
    ...
    adapter.send_multiple_images = AsyncMock(return_value=SendResult(success=True, message_id="image"))
...
@pytest.mark.asyncio
async def test_base_adapter_routes_voice_tagged_telegram_media_tag_to_document_sender(tmp_path, monkeypatch):
    adapter = _MediaRoutingAdapter()
    event = _event()
...
async def test_streaming_delivery_routes_telegram_media_tags_to_document_sender(
    tmp_path, monkeypatch, filename
):
